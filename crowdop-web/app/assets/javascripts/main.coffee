...
class Group extends Backbone.View
    events:
        ...
        "click    .newDatasource"      : "newDatasource"
    initialize: ->
    ...
    newDatasource: (e) ->
        e.preventDefault()
        @el.removeClass("closed")
        r = jsRoutes.controllers.Datasources.add()
        $.ajax
            url: r.url
            type: r.type
            context: this
...
class Project extends Backbone.View
    ...
    deleteProject: (e) ->
        e.preventDefault()
        false if (!confirm "Remove the project?")
        @loading(true)
...
# ---------------------------------------- DataSource
class DataSource extends Backbone.View
    events:
        "click      .delete"    : "deleteDatasource"
    initialize: ->
        @id = @el.attr("data-datasource")
        @name = $(".name", @el).editInPlace
            context: this
            onChange: @renameDatasource
    renameDatasource: (name) ->
        @loading(true)
        r = jsRoutes.controllers.Datasources.rename(@id)
        $.ajax
            url: r.url
            type: r.type
            context: this
    ...
    deleteDatasource: (e) ->
        e.preventDefault()
        false if (!confirm "Remove the data table?")
        @loading(true)
        r = jsRoutes.controllers.Datasources.delete(@id)
        $.ajax
            url: r.url
            type: r.type
            context: this
...
class AppRouter extends Backbone.Router
    ...
    routes:
        ...
        "/datasources/:datasource/data/:page" :   "datasource"
        "/datasources/:datasource/summary/:page":    "datasummary"
        "/tasks/list/:job":     "listtask"
        "/tasks/:task/assignment":      "listassignment"
        "tasks/:task":      "showtask"
    index: ->
        # show dashboard
        $("#main").load "/ #main"
    project: (project) ->
        ...
        $("#main").load "/projects/" + project
    datasource: (datasource, page) ->
        currentApp = @currentApp
        $("#main").load "/datasources/" + datasource + "/data/" + page
    datasummary: (datasource, page) ->
        currentApp = @currentApp
        $("#main").load "/datasources/" + datasource + "/summary/" + page
    listtask: (job) ->
        currentApp = @currentApp
        $("#main").load "/tasks/list/" + job
    listassignment: (task) ->
        currentApp = @currentApp
        $("#main").load "/tasks/" + task + "/assignment"
    showtask: (task) ->
        currentApp = @currentApp
        $("#main").load "tasks/" + task
